---
title: "Plots for Application 3: patient 11827"
author: CZ ID
...
Load metadata
```{r}
meta <- read.delim(paste0("dataset_ID_metadata_", dataset_number, ".tsv")) %>% filter(str_detect(sample_name, "pt11827")) %>% mutate(amr_cov_filter=10) 
```
...
amr_subset = mutate(amr_subset0, CARD_species = ifelse(str_detect(gene_name, "APH\\(3'\\)-IIIa|PC1 beta-lactamase \\(blaZ\\)|msrA|mphC|aad\\(6\\)|sdrM"), "Multiple genera", "Staphylococcus aureus")) %>% 
    mutate(CARD_species = ifelse(str_detect(gene_name, "mecA|mecR1"), "Multiple Staphylococcus species", CARD_species)) %>% 
    mutate(CARD_species = fct_relevel(CARD_species, "Staphylococcus aureus","Multiple Staphylococcus species","Multiple genera")) %>% 
...
filename_list = list.files(path = "./amr_paper_2023_5929", pattern="App3_pt11827_357128_taxon_report", full.names = T) # background is AMR_paper_App3_pt11827_input_mass_background
...
mgs = lapply(names(file_list), function(nm) mutate(file_list[[nm]], sample_name=str_sub(nm, end = -8))) %>% purrr::reduce(bind_rows) %>%
    #    filter(nt_rpm > 10 & nr_rpm > 10 & nt_alignment_length > 50) %>%  # this filter will remove all species
    filter(nt_z_score >=2) %>% 
...
ggplot(mngs1, aes(x=rank, y = nt_rpm)) + geom_point(aes(color = fill), size = 3) + 
    ...
    theme_bw() + xlab("Rank by NT rpM") +   
    theme(panel.grid.minor = element_blank(), legend.position = "none")  + ylab("NT rpM") +  
    scale_x_continuous(breaks = 1:10, labels = 1:10, limits = c(0.5, 8.5))  + ylim(-10, 260) 

ggsave("plot_App3_pt11827_mNGS.pdf", width=3.5, height=3)
...
ggsave("plot_App3_pt11827_AMR.pdf", width = 4, height = 4.1)
```
